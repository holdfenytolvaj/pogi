...
class PgDb extends queryAble_1.QueryAble {
    ...
    init() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (this.config.logger) this.setLogger(this.config.logger);
    ...
    initFieldTypes() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let specialTypeFields = yield this.query(LIST_SPECIAL_TYPE_FIELDS);
            for (let r of specialTypeFields) {
                ...
                if (this.schemas[r.schema_name][r.table_name]) {
                    this.schemas[r.schema_name][r.table_name].fieldTypes[r.column_name] = [3802, 114].indexOf(r.typid) > -1 ? FieldType.JSON : [3614].indexOf(r.typid) > -1 ? FieldType.TSVECTOR : [1082, 1083, 1114, 1184, 1266].indexOf(r.typid) > -1 ? FieldType.TIME : FieldType.ARRAY;
                }
            }
            for (let r of specialTypeFields) {
                switch (r.typid) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    let runStatement = () => {
                        if (statementList.length == currentStatement) {
                            ...
                            pgdb.query(statement).then(() => runStatement(), reject).catch(reject);
                        }
                    };
                    runStatement();
                }).catch(e => {
