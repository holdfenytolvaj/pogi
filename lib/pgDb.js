    ...
    constructor() {
        ...
        for (let schemaName in pgdb.schemas) {
            ...
            for (let tableName in pgdb.schemas[schemaName].tables) {
                schema.tables[tableName] = schema[tableName] = new pgTable_1.PgTable(schema, pgdb.schemas[schemaName][tableName].desc, pgdb.schemas[schemaName][tableName].fieldTypes);
            }
        }
    }
    ...
    initSchemasAndTables() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let r of schemas_and_tables.rows) {
                this.schemas[r.schema] = this[r.schema] = this.schemas[r.schema] || new pgSchema_1.PgSchema(this, r.schema);
                this.schemas[r.schema].tables[r.name] = this.schemas[r.schema][r.name] = new pgTable_1.PgTable(this.schemas[r.schema], r);
            }
        });
    }
    initFieldTypes() {
    ...
    setTypeParser(typeName, parser, schemaName) {
        return __awaiter(this, void 0, void 0, function* () {
            if (schemaName) {
                let oid;
                try {
                    oid = yield this.queryOneField(GET_OID_FOR_COLUMN_TYPE_FOR_SCHEMA, { typeName, schemaName });
                } catch (e) {
                ...
                let list;
                try {
                    list = yield this.queryOneColumn(GET_OID_FOR_COLUMN_TYPE, { typeName });
                } catch (e) {
                    throw Error('Not existing type: ' + typeName);
                }
                list.forEach(oid => {
