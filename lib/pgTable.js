...
export class PgTable extends QueryAble {
    ...
    async insertAndGet(records, options) {
    ...
    ;
    async updateOne(conditions, fields, options) {
    ...
    async updateAndGetOne(conditions, fields, options) {
    ...
    async update(conditions, fields, options) {
    ...
    ;
    async updateAndGet(conditions, fields, options) {
        let { sql, parameters } = this.getUpdateQuery(conditions, fields, options);
        sql += " RETURNING " + (options && options.return && Array.isArray(options.return) ? options.return.map(pgUtils.quoteFieldName).join(',') : '*');
    ...
    ;
    async upsertAndGet(record, options) {
        options = options || {};
        if (!record) {
            throw new Error("insert should be called with data");
        }
    ...
    async deleteAndGet(conditions, options) {
        options = options || {};
        let { sql, parameters } = this.getDeleteQuery(conditions, options);
        sql += " RETURNING " + (options && options.return && Array.isArray(options.return) ? options.return.map(pgUtils.quoteFieldName).join(',') : '*');
        return this.query(sql, parameters);
    }
    async deleteAndGetOne(conditions, options) {
    ...
    async find(conditions, options) {
        ...
        let sql = `SELECT ${pgUtils.processQueryFields(options, this)} FROM ${this.qualifiedName} ${where.where} ${pgUtils.processQueryOptions(options, this)}`;
        return options.stream ? this.queryAsStream(sql, where.params, options) : this.query(sql, where.params, options);
    }
    async findWhere(where, params, options) {
        options = options || {};
        let sql = `SELECT ${pgUtils.processQueryFields(options, this)} FROM ${this.qualifiedName} WHERE ${where} ${pgUtils.processQueryOptions(options, this)}`;
    ...
    async findAll(options) {
        options = options || {};
        let sql = `SELECT ${pgUtils.processQueryFields(options, this)} FROM ${this.qualifiedName} ${pgUtils.processQueryOptions(options, this)}`;
    ...
    async findOneFieldOnly(conditions, field, options) {
        options = options || {};
