"use strict";
const pgdb_operators_1 = require("./pgdb_operators");
const pgdb_1 = require("./pgdb");
var _ = require("lodash");
var util = require("util");
class FieldAndOperator {
}
function generateWhere(conditions, fieldTypes, tableName, placeholderOffset = 0) {
    var result = generate({
        params: [],
        predicates: [],
...
function handleArrayValue(result, fieldAndOperator, value, fieldTypes) {
    ...
    if (fieldTypes[fieldAndOperator.field] == pgdb_1.FieldType.JSON &&
        ['?|', '?&'].indexOf(fieldAndOperator.operator) != -1) {
        result.params.push(value);
        value = util.format("$%s", result.params.length + result.offset);
    }
    else if (fieldTypes[fieldAndOperator.field] == pgdb_1.FieldType.JSON &&
        ['@>', '<@', '&&'].indexOf(fieldAndOperator.operator) != -1) {
    ...
    else if (fieldTypes[fieldAndOperator.field] == pgdb_1.FieldType.ARRAY &&
        ['=', '<>', '<', '>', '<=', '>=', '@>', '<@', '&&'].indexOf(fieldAndOperator.operator) != -1) {
        result.params.push(value);
        value = util.format("$%s", result.params.length + result.offset);
    }
...
function getOp(str) {
    for (let i = 0; i < str.length; i++) {
        if (pgdb_operators_1.default[str.substr(i)]) {
            return str.substr(i);
        }
    }
    return '';
...
function parseKey(key) {
    ...
    let operation = pgdb_operators_1.default[userOp] || {};
    let jsonRegexp = /(-->|->>|#>>)/;
    let field;
    let quotedField;
